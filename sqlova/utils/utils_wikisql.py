...
def get_loader_wikisql(data_train, data_dev, bS, shuffle_train=True, shuffle_dev=False):
    # print("torch dataloader: get_loader_wikisql")
...
def get_num_valid_list(pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, g_wo, header_types):
    """
    g_sc: select column
    g_sa: select aggregator
    g_wn: number of conditions
    g_wc: condition column
    g_wo: condition operator
    """
    # print("g_sc={}, g_sa={}".format(g_sc, g_sa))
    # print("g_wc ={}, g_wo={}".format(g_wc, g_wo))
    # print("header type={}".format(header_types))

    # select_col and sel_agg_op
    cond_ops = ['', 'MAX', 'MIN', 'COUNT', 'SUM', 'AVG']
    pr_cond_ops = []
    print("g_wo={}".format(g_wo))
    print("pr_wo={}".format(pr_wo))
    print(pr_wo == g_wo)
    for x in pr_wo:
        if len(x) == 0:
            pr_cond_ops.append([])
            ...
            pr_cond_ops.append(cond_ops[x[0]])
    # print("sa = {}".format(g_select_aggregators))
    batch_id = 0
    cnt_cond_op_list = []
    # print("g_wc: {}, g_cond_ops: {}".format(g_wc, g_cond_ops))
    # print("sel {} cond {}".format(len(g_sc) == len(g_sa), len(g_wc) == len(g_cond_ops)))
    for cond_column_idx, pr_cond_op in zip(pr_wc, pr_cond_ops):
        if cond_column_idx == [] or pr_cond_op ==[]:
            continue
        elif cond_column_idx[0] >= len(header_types[batch_id]):
            continue
        # print(header_types[batch_id], cond_column_idx,g_cond_op)
        cond_column = header_types[batch_id][cond_column_idx[0]]
        batch_id += 1
        if cond_column[1] == 'text':
            cnt_cond_op_list.append(pr_cond_op in ['', 'COUNT'])
            if pr_cond_op not in ['', 'COUNT']:
                print("text type: {} {} {}".format(cond_column, pr_cond_op, pr_cond_op in ['', 'COUNT']))
        elif cond_column[1] == 'real':
            cnt_cond_op_list.append(pr_cond_op in ['', 'MAX', 'MIN', 'COUNT', 'SUM', 'AVG'])
            # print("real type: {} {} {}".format(cond_column, g_cond_op, g_cond_op in ['', 'MAX', 'MIN', 'COUNT', 'SUM', 'AVG']))

    # print("cnt_aggop_type={}".format(cnt_aggop_type))

    agg_ops = ['=', '>', '<', 'OP']
    pr_select_aggregators = []
    # print("g_sa: {}".format(g_sa))
    for x in pr_sa:
        if x >= len(agg_ops):
            pr_select_aggregators.append('OP')
        else:
            pr_select_aggregators.append(agg_ops[x])
    # print("sa = {}".format(g_select_aggregators))
    batch_id = 0
    cnt_sel_op_list = []
    for sel_column_idx, pr_sel_ag in zip(pr_sc, pr_select_aggregators):
        sel_column = header_types[batch_id][sel_column_idx]
        batch_id += 1
        if sel_column[1] == 'text':
            cnt_sel_op_list.append(pr_sel_ag in ['='])
            # print("text type: {} {} {}".format(sel_column, g_sel_ag, g_sel_ag in ['=']))
        elif sel_column[1] == 'real':
            cnt_sel_op_list.append(pr_sel_ag in ['=', '>', '<', 'OP'])
            # print("real type: {} {} {}".format(sel_column, g_sel_ag, g_sel_ag in ['=', '>', '<']))
    # print("cond={}\nsel={}".format(cnt_cond_op_list, cnt_sel_op_list))
    cnt_op_list=[]
    for cond_op, sel_op in zip(cnt_cond_op_list, cnt_sel_op_list):
        if cond_op and sel_op:
            cnt_op_list.append(True)
        else:
            cnt_op_list.append(False)
    # print(cnt_op_list)
    # exit()
    return cnt_op_list
...
# this part is for constraint reasoning for MDD third task.
...
def get_masks_for_WOP(tb, pr_wc, max_wn=4, n_cond_ops=4, dropout=0.0):
    ...
    for b, pr_wc1 in enumerate(pr_wc):
        for wc_idx, pr_wc11 in enumerate(pr_wc1):
            cond_op_type = tb[b]['types'][pr_wc11]
            if cond_op_type == 'text':
